...
#include "test_config.hpp"
 ...
 TEST_CASE( "Test Interpreter result with literal expressions", "[interpreter]" ) {
   ...
   { // Boolean True
     ...
     Expression result_test = Expression(true);
//     REQUIRE(result == Expression(true));
//       REQUIRE(result.Data.boolean_value == result_test.express.Data.boolean_value);
   }
 ...
 TEST_CASE( "Test Interpreter special form: if", "[interpreter]" ) {
   {
     ...
     Expression result = run(program);
//     REQUIRE(result.express.Data.number_value == 4.);
//     cout << "THIS PASSES THE RESULT = 4 STAGE" << endl;
     REQUIRE(result == Expression(4.));
     REQUIRE(result == 4.);
   }

   {
     ...
     Expression result = run(program);
     //doesnt know what result of expression is!
     REQUIRE(result == Expression(-4.));
     REQUIRE(result == -4.);
   }
 }
 ...
 TEST_CASE( "Test Interpreter special forms: begin and define", "[interpreter]" ) {
...

  {
    ...
    // REQUIRE(result.express.Data.number_value == Expression(2.).express.Data.number_value);
      // REQUIRE(Expression(0.) == Expression(0.));
      cout << "why does this require fail??" << endl;
  }
 }
